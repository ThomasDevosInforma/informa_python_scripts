...
def main(args):
    ...
    print('---------------------------------------------------------------------------------------------------')
    print('GET EMAILS ASSETS FROM ELOQUA REST API ')
    ...
    for final_env in final_envs:
        ...
        API_URL_PATH = "/api/REST/1.0/assets/emails"
        ...
        while True:
            API_URL_PARAMS = f'?count={count}&page={page_number}&depth=partial'
            ...
            items.extend(response.json().get('elements', []))

            total = response.json().get('total', 0)
            
            number_loops = math.ceil(total / count)
            ...
            if page_number < number_loops :
                ...
                utils.write_dataframe_to_csv(pd.DataFrame.from_dict(items), 'data/script_5/email_assets.csv')
                break
